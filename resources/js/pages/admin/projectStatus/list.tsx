...
import { deleteMethod, edit, form, getDetails, list } from "@/routes/admin/projectStatus";
import { BreadcrumbItem, FlashType, ProjectStatus, Project, PaginationResponse } from "@/types";
...
interface AdminProjectStatusListProps {
    projectStatus: PaginationResponse<ProjectStatus>;
}

interface ProjectStatusDetails {
...
export default function AdminProjectStatusList({ projectStatus }: AdminProjectStatusListProps) {
    ...
    const handleDeleteStatus = (id: number) => {
        router.delete(deleteMethod({ projectStatus: id }).url, {
    ...
    const handleOpenDeleteModal = async (status: ProjectStatus) => {
    ...
    const start = (projectStatus.current_page - 1) * projectStatus.per_page + 1;
    const end = Math.min(projectStatus.current_page * projectStatus.per_page, projectStatus.total);

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Status dos projetos" />

            <div className="px-4 py-6">
                <Heading title="Status dos projetos" />
                ...
                <div>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <span className="text-muted-foreground flex-1 text-sm">
                            Mostrando {start}–{end} de {projectStatus.total} status de projeto
                        </span>

                        <Pagination links={projectStatus.links} />
                    </div>
                </div>
            </div>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                ...
                                <div className="mt-2">
                                    <p>Qtde. projetos vinculados: {statusDetails?.projects.length}</p>
                                </div>

                                {statusDetails?.projects.length > 0 && (
                                    <div className="mt-2 text-red-500">
                                        <p className="mb-1">
                                            Não é possível excluir este status porque ele está vinculado aos projetos:
                                        </p>
                                        <ScrollArea className="h-[120px]">
                                            <ul className="list-disc ml-5">
                                                {statusDetails.projects.map((p) => (
